...
Nutzung:
  ...
  python3 tools/import_excel.py data/produkte.csv

Die Quelldatei wird NICHT kopiert — sie liegt bereits im Zielpfad.
Das Script erzeugt daneben data/produkte.json.

Unterstützte Spaltenköpfe (erste Zeile der Datei):
  Pflicht   : Name, Preis
  Optional  : AltPreis, Bild, Badge (Neu/Sale)
  Kategorie : Kategorie (Bett, Matratze, Lattenrost, Nachttisch, Bettkasten, Textil)
  Filter    : Matratzenmass (oder Grösse), Farbe, Material, Produktart (oder Typ),
              Marke, Härtegrad
"""
import csv
import json
